...
public class ClientConversationEntity : IAggregateRoot
{
    ...
    public ValueEnum<ChannelConvertationType> Channel { get; private set; } = ValueEnum<ChannelConvertationType>.Create("Telefono");
    ...
    public static ClientConversationEntity Create(Guid id, Guid clientId, string channel, string contactNumber, DateTime createdAt, string subject, string message, Guid createdBy)
    {
        ...
        return new ClientConversationEntity(id, ValueGuid.Create(clientId,"cliente"), ValueEnum<ChannelConvertationType>.Create(channel), ValuePhone.Create(contactNumber, "Teléfono"), createdAt, ValueClientConversationSubject.Create(subject), ValueClientConversationMessage.Create(message), createdBy);
    }
}
