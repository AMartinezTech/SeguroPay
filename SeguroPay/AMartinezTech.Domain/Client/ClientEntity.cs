...
{ 
    ...
    public DocIdentityTypes DocIdentityType { get; private set; }
    ...
    {
       var (_docIdentityType, _clientType) = ClientEnumValidator.ValidateEnums(docIdentityType,clientType);


        return new ClientEntity(CreateGuid.EnsureId(id),  _docIdentityType , _clientType, docIdentity, ValueClientName.Create(firstName), ValueClientLastName.Create(lastName), phone, ValueEmail.Create(email), observation, addressRef, locationNo, isActive, contactName, contactPhone, ValueGuid.Create(cityId,"City"), ValueGuid.Create(streetId,"Street"));
    }
