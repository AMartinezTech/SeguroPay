...
public class InsuranceAppServices(IInsuranceReadRepository readRepository, IInsuranceWriteRepository writeRepository)
{
    ...
    {
        var result = await _readRepository.GetByIdAsync(id) ?? throw new Exception($"{ErrorMessages.Get(ErrorType.RecordDoesDotExist)} - Insurance");
        return InsuranceMapper.ToDto(result);
    }

    public async Task<List<InsuranceDto>> FilterAsync(Dictionary<string, object?>? filters = null, Dictionary<string, object?>? globalSearch = null)
    {
