...
public class ClientConversationAppService(IClientConversationReadRepository readRepository, IClientConversationWriteRepository writeRepository)
{
    ...
    #region "Read"
    public async Task<List<ClientConversationDto>> FilterAsync(Dictionary<string, object?>? filter = null, Dictionary<string, object?>? globalSearch = null )
    {
        var result = await _readRepository.FilterAsync(filter, globalSearch);
        return ClientConversationMapper.ToDtoList(result);
    }

    public async Task<ClientConversationDto> GetByIdAsync(Guid id)
    {
        var result = await _readRepository.GetByIdAsync(id) ?? throw new Exception($"{ErrorMessages.Get(ErrorType.RecordDoesDotExist)} - ClientConversation");    
