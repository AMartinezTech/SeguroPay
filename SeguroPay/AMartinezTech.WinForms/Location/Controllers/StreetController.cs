...
public class StreetController(StreetApplicationService service)
{
    ...
    public async Task<BindingList<StreetDto>> FilterAsync(Dictionary<string, object?>? filters = null, Dictionary<string, object?>? search = null)
    {
        var result = await _service.FilterAsync(filters, search);
