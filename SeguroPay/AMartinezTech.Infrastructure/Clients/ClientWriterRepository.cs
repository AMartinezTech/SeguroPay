...
public class ClientWriterRepository(string connectionString) : AdoRepositoryBase(connectionString), IClientWriteRepository
{
    public async Task CreateAsync(ClientEntity entity)
    {
        try
        {
            ...
            using var cmd = new SqlCommand("INSERT INTO clients (id, doc_identity_type_id, client_type, doc_identity, first_name, last_name, street_id, city_id, phone, email, observation, location_no, address_ref, contact_name, contact_phone) " +
                "VALUES (@id, @category_id, @doc_identity_type_id, @client_type, @doc_identity ,@first_name, @last_name, @street_id, @city_id, @phone, @email, @observation, @location_no, @address_ref, @contact_name, @contact_phone)", conn);
            cmd.Parameters.AddWithValue("@id", entity.Id);
            cmd.Parameters.AddWithValue("@doc_identity_type_id", entity.DocIdentityType);
            cmd.Parameters.AddWithValue("@client_type", entity.ClientType.Type.ToString());
